...
onMounted(() => {
  fetch("https://pokeapi.co/api/v2/pokemon?limit=302&offset=0")
    .then(res => res.json())
...
const selectedPokemon = async (pokemon) => {
  loading.value = true
  await fetch(pokemon.url).then(res => res.json()).then(res => pokemonSelected.value = res).catch(err => alert(err)).finally(() => loading.value = false)
      ...
      <div class="row mt-4">
        <div class="col-sm-12 col-md-6 mb-4">
          <CardPokemonSelected :name="pokemonSelected?.name" :xp="pokemonSelected?.base_experience"
            :height="pokemonSelected?.height" :image="pokemonSelected?.sprites.other.dream_world.front_default"
            :loading="loading" />
        </div>
          ...
          <div class="card card-list">
            <div class="card-body row">
              ...
              <ListPokemons v-for="pokemon in pokemonsFiltered" :key="pokemon.name" :name="pokemon.name"
                :urlBaseSvg="urlBaseSvg + pokemon.url.split('/')[6] + '.svg'" @click="selectedPokemon(pokemon)" />
            </div>
          </div>
        </div>
      </div>
